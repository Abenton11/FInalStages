...
import com.example.templatemodule2.*;
...
import com.example.templatemodule2.subject;
import java.util.List;
import java.util.ArrayList;


public class main extends Application {
    static public char userType = 'a';
    static public String userPass = "default123";
    static public String username = "Dr. Grace Hopper";
    public static String currentUserEmail = "";
    static public facuilty currentFac;
...
//testing
    ...
    public void start(Stage stage) throws IOException, InvalidFormatException {
        ...
        File f = new File("C:\\code storage\\UMS.xlsx");
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (facultiesSheet.getRow(i + 1) != null) {
              ...
              /*  if (facultiesSheet.getRow(i + 1).getCell(6) == null) {
                ...
                course c = new course();
                if (facultiesSheet.getRow(i + 1).getCell(6) == null) {
                    facuilties[i].courseOffered = c;
                } else {
                    for (int j = 0; j < courses.length; j++)
                    {
                        if (main.courses[j].name.equals(facultiesSheet.getRow(i + 1).getCell(6).toString()))
                        {
                            facuilties[i].courseOffered = main.courses[j];
                        }
                    }
                ...
                facuilties[i].setEmail("");
                facuilties[i].officeLocation = "";
                facuilties[i].courseOffered = emptyCourse;
                facuilties[i].password = "";
            }
            //endregion
        }
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (eventSheet.getRow(i + 1) != null) {
                ...
                if (eventSheet.getRow(i + 1).getCell(5) == null) {
                    ...
                    if (eventSheet.getRow(i + 1).getCell(5).toString() != "") {
    ...
    public void stop() throws IOException, InvalidFormatException {
        ...
        File excelFile = new File("C:\\code storage\\UMS.xlsx");
        ...
        for (int i = 0; i<facuilties.length;i ++) {
            ...
            if (facuilties[i].courseOffered != null) {
                cell.setCellValue(facuilties[i].courseOffered.name);
            }
        ...
        for (int i = 0; i<events.length;i ++) {
            ...
            cell = row.createCell(4);
            cell.setCellValue(events[i].dateTime);
