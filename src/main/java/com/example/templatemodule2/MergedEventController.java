...
import com.example.templatemodule2.Event;
...
public class MergedEventController {
    ...
    public ArrayList<String> registeredEvents = new ArrayList<>();


    @FXML
    public void initialize() {
        // delete the comments when demonstrating user panel

        if (eventList.isEmpty()) {
            Event defaultEvent = new Event(
                    "Orientation Day",                          // name
                    "Welcome to the campus! Meet your peers.",  // description
                    "default.jpg",                              // image path
                    "Main Hall",                                // location
                    "2025-04-15 10:00 AM",                      // date & time
                    100,                                        // capacity
                    "Free"                                      // cost
            );
        ...
        updateEventListView();
        if (main.userType == 's') {  // If user is a student
            userPanel.setVisible(true);
            adminPanel.setVisible(false);
        } 
        else if (main.userType == 'a') {  // If user is an admin
...
/*
       // Collections.sort(eventList, Comparator.comparing(event -> LocalDate.parse(event.getDate())));
        Collections.sort(eventList, Comparator.comparing(event -> {
            // Check if the date is not empty
            if (event.getDate().isEmpty()) {
                return LocalDate.MAX; // Use a placeholder max date or handle it appropriately
            }
    ...
    protected void onRegisterEvent() {
        ...
        result.ifPresent(userData -> {
            ...
            main.currentUserEmail = studentEmail;

            for (Event event : eventList) { // Search for the selected event
                if (selectedEvent.startsWith(event.getName())) {
                    ...
                    emailConfirmation.sendConfirmationEmail(studentEmail);
    ...
    protected void onViewRegisteredEvents() {
        ...
        popupStage.setTitle("Registered Events");

        // Create a VBox to hold the ListView and other controls
        VBox vbox = new VBox(10);
        vbox.setStyle("-fx-padding: 20px; -fx-alignment: center;");

        // Create a ListView to display the registered events
        ListView<String> registeredEventListView = new ListView<>();
        registeredEventListView.setPrefHeight(200);
        registeredEventListView.setPrefWidth(300);

        // Add items to the ListView
        registeredEventListView.getItems().clear();
        if (main.userType == 's') {  // Only students can view their registered events
            for (Event event : eventList) {
                if (event.getRegisteredStudentEmails().contains(main.currentUserEmail)) {
                    System.out.println("Registered event: " + event.getName());  // Debugging line
                    registeredEventListView.getItems().add(event.getName() + " | " + event.getDate());
                }
            }
        }

        // Check if any events are found
        if (registeredEventListView.getItems().isEmpty()) {
            System.out.println("No registered events found for the user.");
        }

        // Add ListView to VBox
        vbox.getChildren().add(registeredEventListView);

        // Create the scene for the popup stage
